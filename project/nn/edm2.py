...
"""
This module contains code from the edm2 repository from NVIDIALabs (Karras et al., 2024).
"""
...
class UNet(nn.Module):
    ...
    def forward(self, x, noise_labels, class_labels = None):
        ...
        # If we are sampling we need to repeat the same sigma value
        # for each image (as we incremently decrease the same value
        # during the sampling process).
        if len(noise_labels.shape) == 0:
            noise_labels = noise_labels.unsqueeze(0).repeat(x.shape[0])
...
class Preconditioning(torch.nn.Module):
    def __init__(self,
        img_resolution,         # Image resolution.
        img_channels,           # Image channels.
        label_dim       = 0,    # Class label dimensionality. 0 = unconditional.
